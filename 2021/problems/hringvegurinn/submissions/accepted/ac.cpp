...
int main() {
    ...
    while(q--) {
        ...
        if(o == 2) {
            cin >> x;
            if(l <= r) sg.range_update(l, r, x);
            else sg.range_update(l, n - 1, x), sg.range_update(0, r, x);
        }
        if(o == 3) {
            ll res = 0;
            if(l <= r) res += sg.query(l, r).x;
            else res += sg.query(l, n - 1).x, res += sg.query(0, r).x;
            cout << res << '\n';
        }
    }
}
