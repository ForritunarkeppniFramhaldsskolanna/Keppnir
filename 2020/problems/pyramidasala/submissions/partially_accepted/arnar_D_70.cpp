...
int get_inorder_index(int val)
{
    return inv_inorder[val];
}
...
int make_tree(Node *&curr, int lo, int hi, int ind)
{
   ...
   int cnt = 1;
   int in_ind = get_inorder_index(preorder[ind]);
...
int main()
{
    ...
    inv_inorder.assign(n+1, 0);
    children.assign(n+1, vi(0));
    rep(i,0,n) cin >> preorder[i];
    rep(i,0,m) cin >> inorder[i], inv_inorder[inorder[i]] = i;
