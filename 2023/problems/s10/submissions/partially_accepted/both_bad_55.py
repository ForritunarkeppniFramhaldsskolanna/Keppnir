...
class Garage(BaseGarage):
    def __init__(self, num_parking_spots, num_companies):
        ...
        self.company_plates = [[] for _ in range(num_companies)]
        self.parked = []
        self.parked_company = [[] for _ in range(num_companies)]
        self.illegal_parked = []

    def register_plate_to_company(self, company_index, plate):
        self.company_plates[company_index].append(plate)

    def get_company_for_plate(self, plate):
        for i, plates in enumerate(self.company_plates):
            if plate in plates:
                return i
        return None
    ...
    def park_for_company(self, company, plate):
        if company is None:
            return
        self.parked_company[company].append(plate)

    def park_not_parking_spot(self, plate):
        self.illegal_parked.append(plate)
        return True
