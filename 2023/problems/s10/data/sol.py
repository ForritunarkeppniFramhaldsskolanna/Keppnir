...
class Garage(BaseGarage):
    def __init__(self, num_parking_spots, num_companies):
        ...
        self.company_spots_current = [0 for _ in range(num_companies)]
        self.plate_to_company = {}
        self.parking_spots = set()
        self.not_parking_spots = set()
    ...
    def is_in_parking_spot(self, plate):
        return plate in self.parking_spots

    def is_in_not_parking_spot(self, plate):
        return plate in self.not_parking_spots

    def num_free_parking_spots(self):
        return self.num_parking_spots - len(self.parking_spots)
    
    def num_free_parking_spots_company(self, company):
        if company is None:
            return 0
        return self.company_spots_limit[company] - self.company_spots_current[company]
    
    def park_for_company(self, company, plate):
        if company is None:
            return
        self.company_spots_current[company] += 1

    def park_not_parking_spot(self, plate):
        self.not_parking_spots.add(plate)
        return True

    def park_parking_spot(self, plate):
        self.parking_spots.add(plate)
        return False
    ...
    def unpark_not_parking_spot(self, plate):
        self.not_parking_spots.remove(plate)

    def unpark_parking_spot(self, plate):
        self.parking_spots.remove(plate)
