...
rows, cols, diag, anti = dict(), dict(), dict(), dict()

def sgn(x):
    return (x > 0) - (x < 0)

def add_point(x, y):
    if y not in rows: rows[y] = SortedSet()
    if x not in cols: cols[x] = SortedSet()
    if x - y not in diag: diag[x - y] = SortedSet()
    if x + y not in anti: anti[x + y] = SortedSet()
    rows[y].add(x)
    cols[x].add(y)
    diag[x - y].add(x + y)
    anti[x + y].add(x - y)
...
def rem_point(x, y):
    ...
    if x in cols:
        ...
        if len(cols[x]) == 0:
            cols.pop(x)
    if x - y in diag:
        diag[x - y].discard(x + y)
        if len(diag[x - y]) == 0:
            diag.pop(x - y)
    if x + y in anti:
        anti[x + y].discard(x - y)
        if len(anti[x + y]) == 0:
            anti.pop(x + y)

def target(posx, posy, goalx, goaly):
    if posx == goalx or posy == goaly:
        return (goalx, goaly)
    mov = min(abs(posx - goalx), abs(posy - goaly))
    return (posx + sgn(goalx - posx) * mov, posy + sgn(goaly - posy) * mov)

def dist(posx, posy, goalx, goaly):
    return max(abs(posx - goalx), abs(posy - goaly))

def sweeps(px, py, qx, qy, tx, ty, dt):
    sgnx, sgny = sgn(tx - px), sgn(ty - py)
    mov = max(abs(px - tx), abs(py - ty))
    mov2 = max(abs(px - qx), abs(py - qy))
    if mov2 > mov:
        return False
    if mov2 < dt:
        return False
...
if __name__ == "__main__":
    ...
    while len(inp) > 0 or len(todo) > 0:
        outp = []
        while len(todo) > 0 and ((todo[0][1] not in cols) or (todo[0][2] not in cols[todo[0][1]])):
            todo.popleft()
        if len(todo) == 0 and len(inp) == 0:
            continue
        if len(todo) == 0 and time < inp[-1][0]:
            time = inp[-1][0]
        while len(inp) > 0 and time >= inp[-1][0]:
            todo.append(inp[-1])
            add_point(inp[-1][1], inp[-1][2])
            inp.pop()
        targ = target(posx, posy, todo[0][1], todo[0][2])
        if targ == (posx, posy):
            outp.append(todo[0])
            todo.pop()
        else:
            sgnx, sgny = sgn(targ[0] - posx), sgn(targ[1] - posy)
            search_in, key, lo, hi = None, None, None, None
            if sgnx == 0: search_in, key = cols, posx
            elif sgny == 0: search_in, key = rows, posy
            elif sgnx == sgny: search_in, key = diag, posx - posy
            else: search_in, key = anti, posx + posy
            if sgnx == 0 and sgny > 0: lo, hi = posy, targ[1]
            elif sgnx == 0 and sgny < 0: lo, hi = targ[1], posy
            elif sgny == 0 and sgnx > 0: lo, hi = posx, targ[0]
            elif sgny == 0 and sgnx < 0: lo, hi = targ[0], posx
            elif sgnx == sgny and sgnx > 0: lo, hi = posx + posy, targ[0] + targ[1]
            elif sgnx == sgny and sgnx < 0: lo, hi = targ[0] + targ[1], posx + posy
            elif sgnx != sgny and sgnx > 0: lo, hi = posx - posy, targ[0] - targ[1]
            elif sgnx != sgny and sgnx < 0: lo, hi = targ[0] - targ[1], posx - posy
            assert lo <= hi
            if key in search_in:
                ind = search_in[key].bisect_left(lo)
                while ind < len(search_in[key]) and search_in[key][ind] <= hi:
                    curx, cury = key, search_in[key][ind]
                    if sgny == 0: curx, cury = cury, curx
                    if sgnx != 0 and sgny != 0 and sgnx != sgny:
                        curx += cury
                        curx //= 2
                        cury = curx - cury
                    if sgn != 0 and sgny != 0 and sgnx == sgny:
                        curx += cury
                        curx //= 2
                        cury = cury - curx
                    dt = dist(posx, posy, curx, cury)
                    outp.append((time + dt, curx, cury))
                    ind += 1
            timestep = max(abs(posx - targ[0]), abs(posy - targ[1]))
            while len(inp) > 0 and time + timestep >= inp[-1][0]:
                if sweeps(posx, posy, inp[-1][1], inp[-1][2], targ[0], targ[1], inp[-1][0] - time):
                    inp[-1] = (time + dist(posx, posy, inp[-1][1], inp[-1][2]), inp[-1][1], inp[-1][2])
                    outp.append(inp[-1])
                else:
                    todo.append(inp[-1])
                    add_point(inp[-1][1], inp[-1][2])
                inp.pop()
            posx, posy = targ[0], targ[1]
            time += timestep
        outp.sort()
        for pt in outp:
            rem_point(pt[1], pt[2])
            sys.stdout.write(str(pt[0]) + " " + str(pt[1]) + " " + str(pt[2]) + "\n")
