...
int main()
{
    ...
    size_t q;
    cin >> q;
    vector<map<string, string>> properties(256 * 256);
    ...
    for (size_t i = 0; i < q; i++) {
        ...
        if (operation == define_op) {
            ...
            cout << defs.size() << endl;
            for (const auto& [key, default_value] : defs) {
                const auto it{ properties[ip].find(key) };
                cout << key << " " << (it == properties[ip].end() ? default_value : it->second) << endl;
            }
        }
        else {
            assert(false);
