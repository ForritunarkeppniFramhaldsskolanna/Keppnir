...
int main() {
    ...
    for(int i = 0; i < q; ++i) {
        ...
        if(op == '+') {
            string k, v;
            cin >> k >> v;
            if(!s2int.count(k)) s2int[k] = cnt++, int2s.push_back(k), defs.push_back(0), deftime.push_back(0);
            if(!o2int.count(v)) o2int[v] = cnt2++, int2o.push_back(v);
            if(!props.count(s2int[k])) deftime[s2int[k]] = i;
            ...
            for(int y : props) {
                ...
                int refresh = deftime[y];
                if(t1 < refresh && t2 < refresh) outp.push_back(make_pair(int2s[y], int2o[defs[y]]));
