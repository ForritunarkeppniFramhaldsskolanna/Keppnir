...
def address_to_int(s):
    if "." in s:
        return int(ipaddress.IPv4Address(s)) + 65535 * 65536 ** 2
    else:
...
for _ in range(q):
    ...
    if query[0] == "+":
        new_prop, new_def = query[1:]
        if new_prop not in prop:
            prop[new_prop] = SortedList()
        default[new_prop] = new_def
    elif query[0] == "-":
        del_prop = query[1]
        del prop[del_prop]
        del default[del_prop]
    elif query[0] == "=":
        a, b, name, value = query[1:]
        a, b = address_to_int(a), address_to_int(b)
        i = prop[name].bisect((a, 0, "")) - 1
        if i >= 0 and prop[name][i][0] < a and prop[name][i][1] > b:
            nw1 = (prop[name][i][0], a - 1, prop[name][i][2])
            nw2 = (b + 1, prop[name][i][1], prop[name][i][2])
            del prop[name][i]
            prop[name].add(nw1)
            prop[name].add(nw2)
        if i >= 0 and prop[name][i][0] < a and prop[name][i][1] >= a:
            nw = (prop[name][i][0], a - 1, prop[name][i][2])
            del prop[name][i]
            prop[name].add(nw)
        i = prop[name].bisect((b + 1, 0, "")) - 1
        if i >= 0 and prop[name][i][0] <= b and prop[name][i][1] > b:
            nw = (b + 1, prop[name][i][1], prop[name][i][2])
            del prop[name][i]
            prop[name].add(nw)
        while True:
            i = prop[name].bisect((a, 0, ""))
            if i < len(prop[name]) and prop[name][i][0] >= a and prop[name][i][1] <= b:
                del prop[name][i]
            else:
                break
        prop[name].add((a, b, value))
    else:
        sys.stdout.write(str(len(prop)))
        sys.stdout.write("\n")
        x = address_to_int(query[1])
        for k in sorted(prop.keys()):
            sys.stdout.write(str(k))
            sys.stdout.write(" ")
            i = prop[k].bisect((x + 1, 0, "")) - 1
            if i < 0 or prop[k][i][1] < x:
                sys.stdout.write(default[k])
            else:
                sys.stdout.write(prop[k][i][2])
            sys.stdout.write("\n")
