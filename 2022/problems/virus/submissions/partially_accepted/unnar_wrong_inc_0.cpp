...
int main() {
  ...
  //cout << endl;
  int ans = 0;  
  int mxDec = 0, mxInc = 0, mxOnes = 0;
  for (int i = n-1; i >= 0; i--) { 
    int r = rev[i];
    if (s[i] == '1') {
      // flip this segment of ones to the segment of ones after it that is biggest (mx2)
      ans = max(ans, compr[r]+compr[r+1]+mxOnes);
      // flip this and the next segment of 0's to get the previous segment of 0's as well
      if (r > 0) ans = max(ans, compr[r-1]+compr[r]+compr[r+1]+compr[r+2]);
      mxOnes = max(mxOnes, compr[r]);
      mxInc = max(mxInc, inc[i]);
      i -= compr[r] - 1;
      continue;
    }
