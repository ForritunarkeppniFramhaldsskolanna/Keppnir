...
def bfs_steps_solver(state):
    if not solvable(state):
        return None
    n, m = get_size(state)
    move_arr = []
    state = execute_moves(state, bfs_solver(state, lambda x: x[0] == tuple(range(1, m+1))), move_arr)
    state = execute_moves(state, bfs_solver(state), move_arr)
    return ''.join(move_arr)
...
if __name__ == "__main__":
    ...
    use_solver(state, bfs_steps_solver)
    """
    print_state(state)
    cmd = read_move()
    while cmd != QUIT:
