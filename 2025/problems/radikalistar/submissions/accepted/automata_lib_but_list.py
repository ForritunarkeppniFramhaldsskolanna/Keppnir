...
class PartitionRefinement(Generic[T]):
    ...
    def __init__(self, n) -> None:
        ...
        S = set(range(1, n + 1))
        self._sets = {id(S): S}
        self._partition = [-1] + [id(S) for x in S]
...
if __name__ == "__main__":
    ...
    partitioning = PartitionRefinement(n)
    for _ in range(q):
        op, *args = input().split()
        if op == "r":
            m, *x = map(int, args)
            ...
            sys.stdout.write(f"{len(partitioning.get_set_ids())}")
            sys.stdout.write("\n")
        elif op == "m":
            set_values = partitioning.get_set_by_id(partitioning.get_id(int(args[0])))
            sys.stdout.write(" ".join(str(x) for x in set_values))
            sys.stdout.write("\n")
