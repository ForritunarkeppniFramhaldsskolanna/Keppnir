...
def predict(original, m):
    SEASONS = min(len(original), WEEK)

    hour_in_day_average = original.seasonal_average(SEASONS)
    seasonality = TimeSeries([hour_in_day_average[i % SEASONS] for i in range(n)])

    residual = original - seasonality
    ...
    for i in range(n, n + m):
        ...
        result.append(seasonality_prediction + base)
    return result
